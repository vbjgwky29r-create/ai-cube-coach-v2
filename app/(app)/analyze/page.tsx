...
export default function AnalyzePage() {
  ...
  const generateOptimal = async () => {
    ...
    if (!trimmed || trimmed.length < 3) {
      setOptimalError('Please enter a scramble first.')
      return
    }

    let seedCubeState: CubeState | null = null
    try {
      seedCubeState = unflattenCubeState(applyScramble(createSolvedCube(), trimmed))
    } catch {
      seedCubeState = null
    }
    ...
    setOptimalResult((prev: any) => ({
      ...
      steps: prev?.steps || 0,
      explanations: prev?.explanations || ['Solving in stages: Cross -> F2L -> OLL -> PLL'],
    }))
    ...
    try {
      const response = await fetch('/api/cube/optimal', {
        ...
        body: JSON.stringify({ scramble: trimmed }),
        signal: controller.signal,
      })

      const data = await response.json().catch(() => null)
      if (!response.ok) {
        setOptimalError(data?.error || 'Failed to generate optimal solution')
        if (data) {
          setOptimalResult((prev: any) => ({
            ...(prev || {}),
            ...data,
            cubeState: data.cubeState || prev?.cubeState || seedCubeState,
            explanations: data.explanations || [data.error || 'No verified full solution yet.'],
          }))
        }
        return
      }
                ...
                <CardContent className="space-y-3">
                  <div className="grid md:grid-cols-3 gap-3">
                    ...
                    <div className="p-3 rounded-lg bg-gradient-to-br from-blue-50 to-transparent border border-blue-100">
                      ...
                      <p className="text-xs text-slate-600">Use keyboard to input solve formula and run AI analysis.</p>
                    </div>
