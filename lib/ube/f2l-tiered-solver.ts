...
import { SLOTS, COLORS, CUBEJS_INDICES, CROSS_EDGES, checkCrossComplete } from './constants'
...
function getLevel(state: string, slot: Slot): LevelResult {
  ...
  if (cornerPos === 'DFR' || cornerPos === 'DFL' || cornerPos === 'DBL' || cornerPos === 'DBR') {
    ...
    cornerOriented = idx[0] === COLORS.D || idx[1] === COLORS.D || idx[2] === COLORS.D
  }
