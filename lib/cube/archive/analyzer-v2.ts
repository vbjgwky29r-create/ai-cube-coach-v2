...
import { solveCube as solveCubeKociemba } from './solver'
import { solveCubeCFOP, formatCFOPSolution, type CFOPSolution } from './cfop-solver'
...
export async function analyzeSolutionDetailed(params: {
  ...
  // 2. 计算参考解法 (使用 CFOP 方法)
  // CFOP 是人类使用的方法，比数学最优解更适合学习
  const cfopSolution = solveCubeCFOP(params.scramble)
  const optimalSteps = cfopSolution.totalSteps

  // 同时计算数学最优解（用于效率对比）
  const mathOptimal = solveCubeKociemba(params.scramble)
  const mathOptimalSteps = mathOptimal.length
  ...
  return {
    ...
    // CFOP 风格的参考解法（人类使用的方法）
    optimalSolution: cfopSolution.fullSolution,
    cfopSolution: {
      cross: {
        moves: cfopSolution.cross.moves,
        steps: cfopSolution.cross.steps,
        description: cfopSolution.cross.description,
      },
      f2l: {
        pairs: cfopSolution.f2l.pairs.map(p => ({
          slot: p.slot,
          moves: p.moves,
          steps: p.moves.split(/\s+/).filter(Boolean).length,
          observation: p.observation,
        })),
        totalSteps: cfopSolution.f2l.totalSteps,
        description: cfopSolution.f2l.description,
      },
      oll: {
        caseNumber: cfopSolution.oll.caseNumber,
        caseName: cfopSolution.oll.caseName,
        moves: cfopSolution.oll.moves,
        steps: cfopSolution.oll.steps,
        description: cfopSolution.oll.description,
      },
      pll: {
        caseNumber: cfopSolution.pll.caseNumber,
        caseName: cfopSolution.pll.caseName,
        moves: cfopSolution.pll.moves,
        steps: cfopSolution.pll.steps,
        description: cfopSolution.pll.description,
      },
      totalSteps: cfopSolution.totalSteps,
    },
    narrative
  }
}
